    ...
    return (
        ...
        >
            <motion.div
                ...
                className="h-full gap-2 block bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 hover:shadow-lg transition-shadow duration-300"
                whileHover="hover"
            >
                {/* Product Image */}
                <Link
                    to={`/product/${product._id}`}
                    className="block relative overflow-hidden flex-shrink-0 w-full"
                >
                    ...
                    {product.stock < 50 && (
                        <div className="absolute top-2 left-2 bg-red-500 text-white text-xs px-2 py-1 rounded-full">
                            {product.stock == 0 ? <><GiEmptyHourglass className="inline-block mr-1" />Out Of Stock</> : <><FaFire className="inline-block mr-1" />Selling Fast</>}
                        </div>
                    )}
              ...
              >
                <MdPayments className="inline-block mr-1" />Buy Now
                            ...
                            <motion.button
                                disabled={product.stock === 0}
                                className={`text-xs min-w-fit bg-gray-200 text-gray-700 px-3 py-1 rounded-full ${product.stock === 0 ? 'cursor-not-allowed' : ''}`}
                                onClick={addToCartHandler}
                                variants={buttonVariants}
                            >
                                <FiShoppingCart className="inline-block mr-1" /> <span className="hidden lg:inilne">Add to Cart</span>
                            </motion.button>
                        </motion.div>
                    </div>
                </div>
